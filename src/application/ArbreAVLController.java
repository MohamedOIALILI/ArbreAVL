...
public class ArbreAVLController {
	...
	private void chercherElem(ActionEvent event) {
		...
		if (ArbreAVL.chercher(aavl, numbre)) {
			...
			label.setText("Ce nombre n'existe pas dans l'arbre");
	...
	private void tracer_aAVL(ArbreAVL a, Comparable<Integer> o) {
		if (a != null) {
			pan.getChildren().clear();
			if (o == null)
	...
	private float tracer_aAVL(float x1, float x2, ArbreAVL a, float y) {
		...
		// après le calcule de la postion du noeud actuel je trace le cercle
	...
	private float tracer_after_search(float x1, float x2, ArbreAVL a, float y, Comparable<Integer> o) {
		float xd = 0, xg = 0;
		int nbFeuille = ArbreAVL.GetnbFeuille(a);
		if (nbFeuille == 0)
			nbFeuille = 1;
